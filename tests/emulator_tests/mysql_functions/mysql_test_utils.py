...
class ProductsWithIdentityDAO(MySqlTestHelper):
    ...
    def create_table(self):
        ...
        Note: This table does NOT include az_func_updated_at column.
        The GetAndAddProducts function explicitly selects only ProductId, Name, Cost
        to avoid timestamp format issues when copying between tables.
        """
        cursor = None
        try:
            self.connect()
