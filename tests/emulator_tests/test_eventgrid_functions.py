...
class TestEventGridFunctions(testutils.WebHostTestCase):
    ...
    # =========================================================================
    # Different Parameter Types Tests (equivalent to C# String, BinaryData, JObject)
    # =========================================================================
    def test_eventgrid_trigger_string_param(self):
        """Test EventGridTrigger with string parameter type.
        
        Equivalent to C# TestEventGridToString_Single
        """
        event_id = f"string-param-{uuid.uuid4()}"
        event = {
            'id': event_id,
            'eventType': 'Microsoft.Test.StringParam',
            'subject': '/test/string/param',
            'eventTime': '2026-03-06T07:00:00Z',
            'data': {'message': 'string param test'},
            'dataVersion': '1.0'
        }

        logger.info(f"Testing EventGrid trigger with string param: {event_id}")
        r = self.webhost.request('POST', 'eventgrid_trigger_string',
                                 data=json.dumps(event),
                                 headers={'Content-Type': 'application/json'},
                                 max_retries=3,
                                 expected_status=200)
        ...
        r = self.webhost.wait_and_request('GET', 'get_eventgrid_string_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], '/test/string/param')
        self.assertEqual(result['param_type'], 'str')
        self.assertIn('raw_event', result)

    def test_eventgrid_trigger_bytes_param(self):
        """Test EventGridTrigger with bytes parameter type.
        
        Equivalent to C# TestEventGridToBinaryData_Single
        """
        event_id = f"bytes-param-{uuid.uuid4()}"
        event = {
            'id': event_id,
            'eventType': 'Microsoft.Test.BytesParam',
            'subject': '/test/bytes/param',
            'eventTime': '2026-03-06T07:00:00Z',
            'data': {'message': 'bytes param test'},
            'dataVersion': '1.0'
        }

        logger.info(f"Testing EventGrid trigger with bytes param: {event_id}")
        r = self.webhost.request('POST', 'eventgrid_trigger_bytes',
                                 data=json.dumps(event),
                                 headers={'Content-Type': 'application/json'},
                                 max_retries=3,
                                 expected_status=200)
        ...
        r = self.webhost.wait_and_request('GET', 'get_eventgrid_bytes_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], '/test/bytes/param')
        self.assertEqual(result['param_type'], 'bytes')
        self.assertGreater(result['raw_length'], 0)

    def test_eventgrid_trigger_dict_param(self):
        """Test EventGridTrigger with dict parameter type.
        
        Equivalent to C# TestEventGridToJObject_Single
        """
        event_id = f"dict-param-{uuid.uuid4()}"
        test_data = {'message': 'dict param test', 'value': 42}
        event = {
            'id': event_id,
            'eventType': 'Microsoft.Test.DictParam',
            'subject': '/test/dict/param',
            'eventTime': '2026-03-06T07:00:00Z',
            'data': test_data,
            'dataVersion': '1.0'
        }

        logger.info(f"Testing EventGrid trigger with dict param: {event_id}")
        r = self.webhost.request('POST', 'eventgrid_trigger_dict',
                                 data=json.dumps(event),
                                 headers={'Content-Type': 'application/json'},
                                 max_retries=3,
                                 expected_status=200)
        ...
        r = self.webhost.wait_and_request('GET', 'get_eventgrid_dict_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], '/test/dict/param')
        self.assertEqual(result['event_type'], 'Microsoft.Test.DictParam')
        self.assertEqual(result['data'], test_data)
        self.assertEqual(result['param_type'], 'dict')
    ...
    # =========================================================================
    # CloudEvent - Different Parameter Types
    # =========================================================================
    def test_cloudevent_trigger_string_param(self):
        """Test CloudEvent trigger with string parameter type.
        
        Equivalent to C# CloudEventParams.TestEventGridToString_Single
        """
        event_id = f"ce-string-param-{uuid.uuid4()}"
        event = {
            'specversion': '1.0',
            'type': 'com.example.string.test',
            'source': '/test/cloudevent/string',
            'id': event_id,
            'time': '2026-03-06T07:00:00Z',
            'subject': 'cloudevent/string/param',
            'datacontenttype': 'application/json',
            'data': {'message': 'CloudEvent string param test'}
        }

        logger.info(f"Testing CloudEvent trigger with string param: {event_id}")
        r = self.webhost.request('POST', 'cloudevent_trigger_string',
                                 data=json.dumps(event),
        ...
        r = self.webhost.wait_and_request('GET', 'get_cloudevent_string_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], 'cloudevent/string/param')
        self.assertEqual(result['type'], 'com.example.string.test')
        self.assertEqual(result['param_type'], 'str')

    def test_cloudevent_trigger_bytes_param(self):
        """Test CloudEvent trigger with bytes parameter type.
        
        Equivalent to C# CloudEventParams.TestEventGridToBinaryData_Single
        """
        event_id = f"ce-bytes-param-{uuid.uuid4()}"
        event = {
            'specversion': '1.0',
            'type': 'com.example.bytes.test',
            'source': '/test/cloudevent/bytes',
            'id': event_id,
            'time': '2026-03-06T07:00:00Z',
            'subject': 'cloudevent/bytes/param',
            'datacontenttype': 'application/json',
            'data': {'message': 'CloudEvent bytes param test'}
        }

        logger.info(f"Testing CloudEvent trigger with bytes param: {event_id}")
        r = self.webhost.request('POST', 'cloudevent_trigger_bytes',
                                 data=json.dumps(event),
                                 headers={'Content-Type': 'application/cloudevents+json'},
                                 max_retries=3,
                                 expected_status=200)
        ...
        r = self.webhost.wait_and_request('GET', 'get_cloudevent_bytes_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], 'cloudevent/bytes/param')
        self.assertEqual(result['type'], 'com.example.bytes.test')
        self.assertEqual(result['param_type'], 'bytes')
        self.assertGreater(result['raw_length'], 0)

    def test_cloudevent_trigger_dict_param(self):
        """Test CloudEvent trigger with dict parameter type.
        
        Equivalent to C# CloudEventParams.TestEventGridToJObject_Single
        """
        event_id = f"ce-dict-param-{uuid.uuid4()}"
        test_data = {'message': 'CloudEvent dict param test', 'value': 99}
        event = {
            'specversion': '1.0',
            'type': 'com.example.dict.test',
            'source': '/test/cloudevent/dict',
            'id': event_id,
            'time': '2026-03-06T07:00:00Z',
            'subject': 'cloudevent/dict/param',
            'datacontenttype': 'application/json',
            'data': test_data
        }

        logger.info(f"Testing CloudEvent trigger with dict param: {event_id}")
        r = self.webhost.request('POST', 'cloudevent_trigger_dict',
                                 data=json.dumps(event),
                                 headers={'Content-Type': 'application/cloudevents+json'},
                                 max_retries=3,
                                 expected_status=200)
        ...
        r = self.webhost.wait_and_request('GET', 'get_cloudevent_dict_triggered',
                                          wait_time=2,
                                          max_retries=10,
                                          expected_status=200)

        result = json.loads(r.text)
        self.assertEqual(result['subject'], 'cloudevent/dict/param')
        self.assertEqual(result['type'], 'com.example.dict.test')
        self.assertEqual(result['data'], test_data)
        self.assertEqual(result['param_type'], 'dict')
